...
test_diff_generation_with_mock_data() {
  ...
  if [ "$status" -eq 1 ]; then  # diff returns 1 when differences found
    ...
    echo "FAIL: Diff generation failed with mock data, status: $status"
    echo "Output: $output"
    return 1
  fi
...
test_archive_inflation_integration() {
  ...
  if ! command -v zip >/dev/null 2>&1; then
    echo "SKIP: zip command not available for archive integration test"
    return 0
  fi
  
  # Create complex test scenario with actual archives
  mkdir -p "$IMG1_DIR/lib"
  mkdir -p "$IMG1_DIR/temp/META-INF"
  mkdir -p "$IMG1_DIR/temp/com/example"
  echo "original content" > "$IMG1_DIR/temp/META-INF/MANIFEST.MF"
  echo "original class" > "$IMG1_DIR/temp/com/example/App.class"
  cd "$IMG1_DIR/temp" && zip -r "$IMG1_DIR/lib/app.jar" . && cd - >/dev/null
  
  echo "config v1" > "$IMG1_DIR/config.properties"
  
  mkdir -p "$IMG2_DIR/lib"
  mkdir -p "$IMG2_DIR/temp/META-INF"
  mkdir -p "$IMG2_DIR/temp/com/example"
  echo "updated content" > "$IMG2_DIR/temp/META-INF/MANIFEST.MF"
  echo "updated class" > "$IMG2_DIR/temp/com/example/App.class"
  cd "$IMG2_DIR/temp" && zip -r "$IMG2_DIR/lib/app.jar" . && cd - >/dev/null
